...
namespace ARR {
	class Guitar : public Base::Guitar {
		public:
			...
			Guitar( const Guitar& g ) : Base::Guitar( g ), phrases( g.phrases ),
				sections( g.sections ), notes( g.notes ), chords(g.chords) { }; 
			...
			const 	std::vector<ARR::Difficulty>& GetDifficulties() const
						{ return difficulties; };
			...
			void	SetDifficulties( const std::vector<ARR::Difficulty>& v )
						{ difficulties = v; };
